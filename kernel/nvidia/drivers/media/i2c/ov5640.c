			      ...
			      const ov5640_reg table[])
{
	...
	pr_info("End of ov5640_write_table\n");	
	return err;
}
...
static struct camera_common_pdata *ov5640_parse_dt(struct i2c_client *client)
{
	...
	if (gpio < 0) {
		dev_err(&client->dev, "pwdn-gpios not found %d\n", gpio);
		gpio = 0;
	}
			...
			const struct i2c_device_id *id)
{
	...
	common_data->frmfmt = &ov5640_frmfmt[0];
